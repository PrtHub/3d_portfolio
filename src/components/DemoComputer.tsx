...
const DemoComputer = (props: Props) => {
  const group = useRef<null | any>(null);
  const { nodes, materials } = useGLTF('/models/computer.glb');
  // const { actions } = useAnimations(animations, group);

  const [textureType, setTextureType] = useState<'image' | 'video' | null>(null);
  const [texture, setTexture] = useState<THREE.Texture | null>(null);

  useEffect(() => {
    const determineTextureType = () => {
      const extension = props.texture.split('.').pop()?.toLowerCase();
      if (['jpg', 'jpeg', 'png', 'gif', 'webp'].includes(extension || '')) {
        setTextureType('image');
      } else if (['mp4', 'webm', 'ogg'].includes(extension || '')) {
        setTextureType('video');
      } else {
  ...
  useEffect(() => {
    if (textureType === 'image') {
      const loadTexture = async () => {
        const imageTexture = await useTexture(props.texture);
